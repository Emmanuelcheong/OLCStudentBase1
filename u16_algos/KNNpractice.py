...
weather_records = {}
for day in day_ids:
    weather_records[day] = weather_data[flag]
    flag += 1
...
wind_speed = int(input("What is the wind_speed"))
shortest_distance = 99999999999999999
predicted_weather_type = ""
nearest_id = ""
for id, data in weather_records.items():
    existing_temp = float(data[0])
    existing_humi = data[1]
    existing_wind_speed = data[2]
    distance = math.sqrt((temperature - existing_temp)**2 + (humidity - existing_humi) **2 + (existing_wind_speed - wind_speed) ** 2)
    print(distance)
    print(f"Distance from {id} is {distance}")
    if distance < shortest_distance:        
        shortest_distance = distance
        nearest_id = id
        predicted_weather_type = data[3]
print(f"The nearest day id is {nearest_id}")
print(f"The predicted weather type is {predicted_weather_type}")
